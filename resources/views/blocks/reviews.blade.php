...
<section
	...
	@class([ 'b-reviews relative -smt hidden' ,
	...
	<div class="__wrapper c-main">
		<div class="__content">
			...
			<div data-gsap-element="header" class="__wrapper block w-full md:w-1/2 pb-10">
				<h3 class="">{{ $header }}</h3>
				<div data-gsap-element="txt" class="m-header">{!! $txt !!}</div>
			</div>
			@endif
			...
			<div class="swiper reviews-swiper !overflow-visible">
				<div data-gsap-element="swiper" class="swiper-wrapper">
					...
					<div class="swiper-slide">
						<div class="__card relative bg-background radius h-full">

							<div class="relative z-10 flex flex-col gap-4 h-full pt-10 p-8 mt-6">
								@if(!empty($card['txt']))
								<div class="review-content-wrapper">
									<div class=""><img src="{{ get_template_directory_uri() }}/resources/images/quote.svg" /></div>
									<p class="text-h5 mt-6 block">{!! $card['header'] !!}</p>
								...
								@if(!empty($card['reference']['url']))
								<div class="lightbox-gallery">
									<x-button
										:href="$card['reference']['url']"
										variant="outline-small"
										:data-caption="($card['reference']['caption'] ?? '') ?: ($card['name'] ?? '')">
										Zobacz referencje
									</x-button>
								</div>
								@endif
								...
								<div class="flex flex-col lg:flex-row items-start lg:items-center gap-2 mt-auto">
									<img src="{{ $card['image']['url'] }}" alt="{{ $card['image']['alt'] ?? '' }}" class="border border-gray-200 rounded-lg bg-white w-36 h-16 object-contain p-4" />
									<div class="__who">
