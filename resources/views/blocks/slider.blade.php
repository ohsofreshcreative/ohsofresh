	...
	<div class="c-main">
		...
		<div class="__header flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-14">
			<div class="w-full md:w-1/2">
				...
				<h2 data-gsap-element="header">{{ $slider_title }}</h2>
				@endif
				@if (!empty($slider_txt))
				<p data-gsap-element="txt" class="m-header">{{ $slider_txt }}</p>
				@endif
			</div>
			@if (!empty($slider_btn))
			<div data-gsap-element="btn" class="shrink-0">
				<x-button :href="$slider_btn['url']" variant="outline" :target="$slider_btn['target'] ?? ''">{{ $slider_btn['title'] }}</x-button>
			</div>
			@endif
		</div>
		@endif
		<div data-gsap-element="slider" class="swiper slider-standard relative !overflow-visible z-20">
			<div class="swiper-wrapper">
				@foreach ($slides as $slide)
				<div class="swiper-slide">
					<article class="__card">
						<a href="{{ $slide['url'] }}" class="__link group relative block overflow-hidden min-h-[400px] sm:min-h-0 aspect-[6/5] radius">
							...
							<div class="__content absolute inset-x-0 bottom-0 z-10 flex items-end justify-between gap-6 p-6 md:p-8 text-white">
								<h3 class="__title text-h5">{!! $slide['title'] !!}</h3>
			...
			<div class="__prev absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/2 rounded-full bg-primary h-14 w-14 flex items-center justify-center cursor-pointer transition-all duration-400 z-10 hover:scale-110">
			...
			<div class="__next absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 rounded-full bg-primary h-14 w-14 flex items-center justify-center cursor-pointer transition-all duration-300 z-10 hover:scale-110">
				<svg xmlns="http://www.w3.org/2000/svg" width="13" height="12" viewBox="0 0 13 12" fill="none">
